...
CROP_INFO = {
    "Wheat": {"uName": "Wheat / گندم", "moisture": "68%", "ph": "6.5", "rate": "Rs. 3,900", "trend": "+2.4%", "season": "Rabi (Nov–Apr)", "tips": "گندم کو بروقت آبپاشی اور نائٹروجن کھاد دیں۔ زنگ کی نگرانی کریں۔"},
    "Cotton": {"uName": "Cotton / کپاس", "moisture": "45%", "ph": "7.2", "rate": "Rs. 8,500", "trend": "+1.8%", "season": "Kharif (May–Oct)", "tips": "سفید مکھی اور بال ورم سے بچاؤ کے لیے باقاعدہ سپرے کریں۔"},
    "Maize": {"uName": "Maize / مکئی", "moisture": "55%", "ph": "6.0", "rate": "Rs. 2,400", "trend": "-0.5%", "season": "Kharif / Spring", "tips": "مکئی کو اچھی نکاسی والی مٹی اور متوازن کھاد درکار ہے۔"},
    "Sugarcane": {"uName": "Sugarcane / گنا", "moisture": "75%", "ph": "6.8", "rate": "Rs. 425", "trend": "Stable", "season": "Year-round (planted Feb–Mar)", "tips": "گنے کو زیادہ پانی اور پوٹاش کھاد پسند ہے۔"},
}
...
@app.get("/")
def root():
    splash = FRONTEND_DIR / "01_splash.html"
    if splash.exists():
        return FileResponse(splash, media_type="text/html")
...
def index_html():
    ...
    if splash.exists():
        return FileResponse(splash, media_type="text/html")
    return HTMLResponse("<h1>AgriPak</h1><p>Frontend not found.</p>")


@app.get("/api")
def api_info():
    return {"service": "AgriPak / AgriVoice API", "mode": "demo" if DEMO_MODE else "live", "docs": "/docs", "frontend": "/"}


@app.post("/api/auth/signup")
def signup(body: SignupRequest):
    conn = get_conn()
    existing = conn.execute("SELECT id FROM users WHERE contact = ?", (body.contact.strip(),)).fetchone()
    if existing:
        conn.close()
    ...
    cur = conn.execute(
        ...
        (body.name.strip(), body.contact.strip(), hash_password(body.password), "Wheat", datetime.now().isoformat(timespec="seconds")),
    )
    user_id = cur.lastrowid
    conn.commit()
    conn.close()
    token = create_token(user_id)
    return {"ok": True, "message": "Account created", "token": token, "user": {"id": user_id, "name": body.name.strip(), "contact": body.contact.strip(), "selected_crop": "Wheat"}}


@app.post("/api/auth/signin")
def signin(body: SigninRequest):
    conn = get_conn()
    row = conn.execute("SELECT * FROM users WHERE contact = ?", (body.contact.strip(),)).fetchone()
    conn.close()
    ...
    return {"ok": True, "token": token, "user": {"id": row["id"], "name": row["name"], "contact": row["contact"], "selected_crop": row["selected_crop"] or "Wheat"}}
...
def forgot_password(contact: str = Form(...)):
    ...
    return {"ok": True, "message": f"Password reset link would be sent to {contact} (demo — not sent).", "user_name": row["name"]}


@app.get("/api/profile")
def profile(user: dict = Depends(require_user)):
    return {"id": user["id"], "name": user["name"], "contact": user["contact"], "selected_crop": user.get("selected_crop") or "Wheat", "created_at": user.get("created_at")}
...
def chat(body: ChatRequest, user: Optional[dict] = Depends(get_user_from_token)):
    ...
    conn = get_conn()
    conn.execute("INSERT INTO questions (user_id, timestamp, question, answer) VALUES (?, ?, ?, ?)", (user_id, datetime.now().isoformat(timespec="seconds"), body.message, answer))
    conn.commit()
    conn.close()
...
async def leaf_scan(file: UploadFile = File(...), user: Optional[dict] = Depends(get_user_from_token)):
    ...
    if "error" not in result:
        ...
        conn = get_conn()
        conn.execute(
            """INSERT INTO diagnoses (user_id, timestamp, image_path, crop, disease, treatment, confidence) VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (user_id, datetime.now().isoformat(timespec="seconds"), str(save_path), result.get("crop", ""), result.get("disease", ""), result.get("treatment", ""), result.get("confidence", "")),
        )
        conn.commit()
        conn.close()
    return {"ok": "error" not in result, "result": result, "image_saved": save_name, "mode": "demo" if DEMO_MODE else "live"}


@app.get("/api/weather")
def weather(latitude: float = 31.5204, longitude: float = 74.3587, user: Optional[dict] = Depends(get_user_from_token)):
    snapshot = get_local_snapshot(latitude, longitude)
    w = snapshot.get("weather", {})
    if "error" not in w and user:
        conn = get_conn()
        conn.execute(
            """INSERT INTO weather_checks (user_id, timestamp, latitude, longitude, temperature_C, humidity_percent) VALUES (?, ?, ?, ?, ?, ?)""",
            (user["id"], datetime.now().isoformat(timespec="seconds"), latitude, longitude, w.get("temperature_C"), w.get("humidity_percent")),
        )
        conn.commit()
        conn.close()
    return snapshot
...
def history(user: dict = Depends(require_user)):
    conn = get_conn()
    diagnoses = [dict(r) for r in conn.execute("SELECT id, timestamp, crop, disease, treatment, confidence FROM diagnoses WHERE user_id = ? ORDER BY id DESC LIMIT 50", (user["id"],)).fetchall()]
    questions = [dict(r) for r in conn.execute("SELECT id, timestamp, question, answer FROM questions WHERE user_id = ? ORDER BY id DESC LIMIT 50", (user["id"],)).fetchall()]
    weather_rows = [dict(r) for r in conn.execute("SELECT id, timestamp, temperature_C, humidity_percent, latitude, longitude FROM weather_checks WHERE user_id = ? ORDER BY id DESC LIMIT 20", (user["id"],)).fetchall()]
    conn.close()
...
def list_fields(user: dict = Depends(require_user)):
    conn = get_conn()
    rows = [dict(r) for r in conn.execute("SELECT id, name, crop, area, location, created_at FROM fields WHERE user_id = ? ORDER BY id DESC", (user["id"],)).fetchall()]
    conn.close()
    return {"fields": rows}
...
def add_field(body: FieldCreate, user: dict = Depends(require_user)):
    ...
    cur = conn.execute(
        ...
        (user["id"], body.name, body.crop, body.area, body.location, datetime.now().isoformat(timespec="seconds")),
    )
...
@app.get("/api/health")
def health():
    return {"status": "ok", "demo_mode": DEMO_MODE, "gemini": not DEMO_MODE, "pillow": PIL_AVAILABLE, "gtts": GTTS_AVAILABLE}
...
@app.get("/api_config.js")
def serve_api_config():
    f = FRONTEND_DIR / "api_config.js"
    if f.exists():
        return FileResponse(f, media_type="application/javascript")
    raise HTTPException(404, "api_config.js not found")


@app.get("/{page_name}.html")
def serve_html_page(page_name: str):
    safe = page_name.replace("..", "").replace("/", "").replace("\\", "")
    f = FRONTEND_DIR / f"{safe}.html"
    if f.exists() and f.is_file():
        return FileResponse(f, media_type="text/html")
    raise HTTPException(404, f"Page not found: {page_name}.html")
